    ...
    calculatePrice(course: Course, pricing: Pricing, paymentMethod: PaymentMethod): number {
        switch (course.type) {
            ...
            case CourseType.session:
                return paymentMethod === PaymentMethod.APPLE ? (pricing.sessionApplePrice ?? 0) : PaymentMethod.ANDROID ? (pricing.sessionAndroidPrice ?? 0) : (pricing.sessionWebPrice ?? 0)
            case CourseType.tutorial:
                return paymentMethod === PaymentMethod.APPLE ? (pricing.tutorialApplePrice ?? 0) : PaymentMethod.ANDROID ? (pricing.tutorialAndroidPrice ?? 0) : (pricing.tutorialWebPrice ?? 0)
            default:
                return 0;
        }
    }
