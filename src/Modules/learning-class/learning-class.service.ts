...
export class LearningClassService {
    ...
    async startLive(user: User, body: StartLiveDTO) {
        ...
        if (existsClass) {
            ...
            const teacherToken = RtcTokenBuilder.buildTokenWithUid(Agora.appId, Agora.appCertificate, lesson.OId, 1, RtcRole.PUBLISHER, privilegeExpiredTs);
            const studentToken = RtcTokenBuilder.buildTokenWithUid(Agora.appId, Agora.appCertificate, lesson.OId, 1, RtcRole.SUBSCRIBER, privilegeExpiredTs);
