...
export class UserService {
  ...
  constructor(
    ...
    public BankAccountModel: Model<BankAccountDocument>,
    @InjectModel(Wallet.name) public WalletModel: Model<WalletDocument>,
    @InjectModel(StudentReview.name)
    public StudentReviewModel: Model<StudentReviewDocument>,
  ...
  async getBankAccounts(req: any) {
    let teacher = await this.findOne(req.user._id);
    if (!teacher) throw new BadRequestException('no user found');
    return teacher.bankAccounts;
  }
  ...
  async withDrawCash(req: any, accountId: string, amount: number) {
    ...
    let balance = teacher.wallet.reduce(
      (acc, wall) =>
        acc +
  ...
  async approveTransaction(teacherId: string, walletId: string) {
    ...
    if (!teacher || !wallet) throw new BadRequestException('Check sent IDs');
  ...
  async createWalletForCheckout(checkoutSaved: CheckoutDocument) {
    ...
    await this.UserModel.updateOne(
      { _id: checkoutSaved.course.teacher['_id'] },
      checkoutSaved.course.teacher,
    );
  }
  ...
  async deleteBankAccount(req: any, accountId: string) {
    let teacher = await this.findOne(req.user._id);
    teacher.bankAccounts.splice(
      teacher.bankAccounts.findIndex((acc) => acc['_id'] === accountId),
      1,
    );
  ...
  async getStudentReviews(req: any, studentId: string, subjectId: string) {
    ...
    let reviews = student.studentReviews.filter(
      (rev) => rev?.course?.subject['_id'].toString() === subjectId,
    );
    ...
    let thisMonthReviews = reviews.filter(
      (rev) =>
        rev.valueDate >= moment().startOf('month').unix() * 1000 &&
    ...
    if (thisMonthReviews.length > 0)
      reviewResponse[thisMonthString] = {
        attendance:
          (thisMonthReviews.filter((rev) => rev.attendance === true).length /
            thisMonthReviews.length) *
          100,
        grades:
          thisMonthReviews.reduce((acc, rev) => acc + rev.grades, 0) /
          thisMonthReviews.length,
        performance:
          thisMonthReviews.reduce((acc, rev) => acc + rev.performance, 0) /
          thisMonthReviews.length,
        understanding:
          thisMonthReviews.reduce((acc, rev) => acc + rev.understanding, 0) /
          thisMonthReviews.length,
      };
    ...
    if (lastMonthReviews.length > 0)
      reviewResponse[lastMonthString] = {
        attendance:
          (lastMonthReviews.filter((rev) => rev.attendance === true).length /
            lastMonthReviews.length) *
          100,
        grades:
          lastMonthReviews.reduce((acc, rev) => acc + rev.grades, 0) /
          lastMonthReviews.length,
        performance:
          lastMonthReviews.reduce((acc, rev) => acc + rev.performance, 0) /
          lastMonthReviews.length,
        understanding:
          lastMonthReviews.reduce((acc, rev) => acc + rev.understanding, 0) /
          lastMonthReviews.length,
      };
